...
class GitHubClient:
    ...
    def add_related_comment(
        self,
        issue_number: int,
        original_issue_number: int,
    ) -> bool:
        marker = f"{COMMENT_MARKER_PREFIX}related-{original_issue_number} -->"
        if self._has_comment(issue_number, marker):
            return False
        ...
        self._request(
            ...
            json={
                "body": (
                    ...
                    f"#{original_issue_number}. Please review it and add any "
                    "additional details there if relevant."
                )
            },
            expected_statuses={201},
        )
...
def find_duplicate(
    ...
    return DuplicateMatch(
        issue=existing_issues[best_index],
        score=best_score,
        tier=tier,
    )
...
def run(title: str, body: str, repository: str) -> None:
    ...
    try:
        ...
        if match is None:
            logging.info(
                "No issue exceeded the %.2f similarity threshold.", DUPLICATE_THRESHOLD
            )
            return

        logging.info(
            "Issue #%d matched issue #%d with similarity %.4f (tier: %s).",
            issue_number,
            match.issue.number,
            match.score,
            match.tier,
        )
        if match.tier == "duplicate":
            ...
            comment_added = client.add_related_comment(issue_number, match.issue.number)
            if comment_added:
                logging.info("Added a related-issue comment to issue #%d.", issue_number)
            else:
                logging.info("The related-issue comment already exists.")
    finally:
        client.close()
